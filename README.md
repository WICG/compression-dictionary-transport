...
* **type** - Dictionary format. Defaults to `raw` but can allow for format-specific serialized dictionaries. If a client receives a dictionary format that it does not understand, it MUST ignore the dictionary and not attempt to use it. Possible values are `raw` for raw bytestream dictionaries, `br` for brotli-specific serialized dictionaries and `zstd` for Zstandard structured dictionaries. If a format-specific dictionary is used, only the applicable content-encodings should be advertised (i.e. don't send `sbr` in the accept-encoding when using a `zstd` dictionary).
