...
* **match** - URL-matching pattern for the dictionary to apply to. *Required*. This is a patternString for a [URLPattern](https://wicg.github.io/urlpattern/) `URLPattern(patternString, baseURL)` constructor where the baseURL is the URL of the request and where support for [regexp](https://wicg.github.io/urlpattern/#token-type-regexp) tokens is disabled. URLPattern allows for absolute or relative URLs and the resulting pattern MUST be same-origin as the request. e.g. `/app1/main*` will match `https://www.example.com/app1/main_12345.js` and `main*` in response to `https://www.example.com/app1/main_1.js` will match `https://www.example.com/app1/main.xyz.js`.
